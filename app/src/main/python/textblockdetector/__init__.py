...
class TextDetector:
    ...
    def __init__(self, model_path, input_size=1024, device='cpu', half=False, nms_thresh=0.35, conf_thresh=0.4,
        ...
        cuda = device == 'cuda'
        # self.session = onnxruntime.InferenceSession("comictextdetector.pt.onnx")
    ...
    def preprocessing(self, img):
        ...
        return img_in, ratio, dw, dh, im_h, im_w
    ...
    #                                            to_tensor=True)
    #     im_h, im_w = img.shape[:2]
    #     blks, mask, lines_map = self.session.run(None, {self.session.get_inputs()[0].name: img_in.numpy()})
    #     resize_ratio = (im_w / (self.input_size[0] - dw), im_h / (self.input_size[1] - dh))
    #     blks = postprocess_yolo(blks, self.conf_thresh, self.nms_thresh, resize_ratio)
    #     mask = postprocess_mask(mask)
    #     lines, scores = self.seg_rep(self.input_size, lines_map)
